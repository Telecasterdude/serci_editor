...
function get_args_from_inputs(selector_string, list_of_inputs) {
    ...
    if (selector_string === 'RERCI ARTICLE') {
        ...
        //File Upload
        if (selected_file === undefined && list_of_inputs['link'].value === "") {
            alert("You have not selected a file to upload or provided a link. Please select a file to upload or provide a link.");
            return null;
        } else {
            selected_file = (list_of_inputs['link'].value !== "") ? list_of_inputs['link'].value : selected_file;
            args.push(selected_file);
        }
    }
...
function send_to_python(args, script_filename) {
    /* Takes a list of arguments and the filename of a script in the python_scripts folder. Then sends the 
       arguments to the python script. The reponse from the python script is printed to the console. */
    ...
    python_shell.on('message', (message) => {console.log(message)});
}
